...
The native Android app is a **React Native (Expo SDK 57) application** under `mobile/`.
...
- Expo CLI (`npx expo`), packages under `mobile/package.json`
...
Then create `mobile/android/local.properties` (it is git-ignored):
...
- **`EXPO_PUBLIC_API_URL`** is inlined by `babel-preset-expo` **at bundle time** (`mobile/src/config.ts`).
...
- **`react_native_dev_server_ip`:** the RN gradle plugin bakes the machine's LAN IP into
  `resources.arsc` unless overridden. `mobile/android/gradle.properties` sets
...
- Keystore: `D:\mindEase-ai\keystore\mindease-release.keystore` (alias `mindease-release`)
- Passwords: `D:\mindEase-ai\keystore\storepass.txt`
- Gradle wiring: `mobile/android/keystore.properties` (absolute paths, git-ignored)
...
Branding/identity lives in `mobile/app.json`:
...
Icons/splash are generated by `mobile/scripts/generate-assets.ps1` (re-run to regenerate).